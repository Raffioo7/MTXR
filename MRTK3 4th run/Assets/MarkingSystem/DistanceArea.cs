...
public class MeasurementHandler_MRTK3 : MonoBehaviour
{
    ...
    public Color distanceColor = Color.yellow;
    ...
    void Start()
    {
        ...
        if (dotHandler == null)
        {
            Debug.LogError("MeasurementHandler: Could not find DotPlacementHandler_MRTK3 script!");
            return;
        }
        ...
        if (debugMode)
            Debug.Log("MeasurementHandler: Initialized successfully");
    }
    
    void CreateDefaultLabelPrefab()
    {
        // Create a simple text label
        labelPrefab = new GameObject("MeasurementLabel");
    ...
    {
        if (debugMode)
            Debug.Log($"MeasurementHandler: Loop {loopIndex + 1} closed, calculating final measurements");
        ...
        if (debugMode)
        {
            ...
            Debug.Log($"MeasurementHandler: Loop {loopIndex + 1} - Total Distance: {FormatDistance(totalDistance)}, Area: {FormatArea(area)}");
        }
    }
    ...
    {
        if (debugMode)
            Debug.Log($"MeasurementHandler: New loop {loopIndex + 1} started");
    }
    ...
    void CreateCurrentDistanceLabel()
    {
        ...
        label.name = $"CurrentLoop_Distance_{currentLoopDistanceLabels.Count + 1}";
    ...
    void CreateAreaLabel(int loopIndex, float area)
    {
        ...
        areaLabel.name = $"Loop{loopIndex + 1}_Area";
    ...
    Vector3 CalculateLoopNormal(List<Vector3> positions)
    {
        if (positions.Count < 3) return Vector3.up;
        ...
        // If the normal is too small, use fallback
    ...
    Vector3 GetCameraFacingDirection(Vector3 worldPosition)
    {
        // Try to get camera direction
        Vector3 cameraDirection = Vector3.forward; // Fallback
        
        if (Camera.main != null)
        {
            cameraDirection = (Camera.main.transform.position - worldPosition).normalized;
        }
        else
        {
            ...
            foreach (Camera cam in cameras)
            {
                ...
                {
                    cameraDirection = (cam.transform.position - worldPosition).normalized;
                    break;
                }
            }
        }
        ...
        if (cameraDirection.magnitude < 0.001f)
        {
            cameraDirection = Vector3.up; // Ultimate fallback
        }
    ...
    public void ToggleMeasurementLabels()
    {
        if (labelsParent != null)
        {
            ...
            if (debugMode)
                Debug.Log($"MeasurementHandler: Labels are now {(!isActive ? "visible" : "hidden")}");
        }
    }
    ...
    public void ToggleUnits()
    {
        ...
        if (debugMode)
            Debug.Log($"MeasurementHandler: Switched to {(useMetricUnits ? "metric" : "imperial")} units");
    }
    ...
    public void ClearAllMeasurements()
    {
        ...
        if (debugMode)
            Debug.Log("MeasurementHandler: Cleared all measurements");
    }
    ...
    void OnDestroy()
    {
        ...
        if (labelPrefab != null && labelPrefab.name == "MeasurementLabel")
        {
            Destroy(labelPrefab);
        }
    }
